...
const UsageReport = ({ token }) => {
  ...
  // ✅ Initialize search and page from URL query params ONCE when component mounts
  useEffect(() => {
    const poParam = searchParams.get('po') || searchParams.get('po_number');
    const pageParam = parseInt(searchParams.get('page'), 10) || 1;

    if (poParam) setSearch(poParam);
  ...
  // helper that returns a promise so we can await logo conversion when needed
  const convertImageToBase64Promise = (imagePath) =>
  ...
  const fetchData = async () => {
    try {
      ...
      if (search) url += `&po_number=${encodeURIComponent(search)}`;
      ...
      const usageResults = usageRes.data.results || [];

      setUsages(usageResults);
      setTotalPages(
        Math.ceil((usageRes.data.count || usageResults.length) / itemsPerPage)
      );
  ...
  const fetchAllUsages = async () => {
    ...
    if (search) params.append('po_number', search);
    if (fromDate) params.append('from_date', fromDate);
    if (toDate) params.append('to_date', toDate);

    let page = 1;
    const all = [];
    const pageSizeTry = 100; // expected page size (matches itemsPerPage)
    try {
      while (true) {
        params.set('page', page);
        // Optional: try a larger page_size if your API supports it
        // params.set('page_size', 1000);

        const url = `/usages/?${params.toString()}`;
        ...
        // If DRF-style pagination, stop when next is null
        if (res.data && typeof res.data.next !== 'undefined' && !res.data.next) {
          break;
        }

        // If results length < page size then last page
        if (results.length === 0 || results.length < pageSizeTry) break;
  ...
  const downloadPDF = async () => {
    try {
      ...
      // Ensure logo is ready
      let logoData = logoBase64;
      if (!logoData) {
        try {
          logoData = await convertImageToBase64Promise(logo);
          setLogoBase64(logoData);
        } catch (err) {
          console.warn('Could not convert logo to base64, continuing without logo', err);
          logoData = '';
        }
      }

      // Fetch all usages with filters
      const allUsages = await fetchAllUsages();
      ...
      const groupedByPO = allUsages.reduce((acc, u) => {
      ...
      Object.entries(groupedByPO).forEach(([poNumber, usagesGroup], idx) => {
        ...
        if (logoData) {
          try {
            doc.addImage(logoData, 14, 8, 25, 12);
          } catch (err) {
        ...
        const inverterDetails = `${first.inverter_given_start_name || ''} ${
          first.inverter_model || ''
        } ${first.client_name || ''} ${first.location_name || ''}`;
        ...
        autoTable(doc, {
          ...
          body: [
            ...
            [
              'Hybrid Generator Fuel Saved ',
              `${hybridFuelSavedRuntime.toFixed(2)} `,
              `€${hybridFuelSavedCost.toFixed(2)}`,
            ],
          ],
          styles: { fontSize: 8 },
          theme: 'grid',
        ...
        // Ensure we have lastAutoTable available; autoTable sets doc.lastAutoTable after call
        autoTable(doc, {
        ...
        for (let i = 1; i <= pageCount; i++) {
          ...
          doc.text(`Page ${i} of ${pageCount}`, 200 - 14, 290, {
            align: 'right',
          });
        }
      });
      ...
      const filename = `${
        firstUsage.inverter_given_start_name || 'Inverter'
      }_${(firstUsage.location_name || 'Location').replace(/\s+/g, '_').trim()}.pdf`;
      doc.save(filename);
      setIsGeneratingPdf(false);
    } catch (err) {
  ...
  return (
    <MDBContainer className="py-4">
      ...
      <div className="d-flex flex-wrap gap-3 mb-3 align-items-end">
        ...
        <MDBBtn color="danger" onClick={downloadPDF} disabled={isGeneratingPdf}>
