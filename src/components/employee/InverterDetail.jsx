...
const InverterDetail = () => {
  const { id } = useParams(); // inverter id from URL
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 mt-4 mb-5">
      ...
      <MDBCard className="mb-4 shadow-sm">
        <MDBCardBody>
          <MDBCardTitle className="fs-5 mb-3">Current Unit Details</MDBCardTitle>
          {current_order ? (
            <div className="row">
              <div className="col-md-4">
                <MDBCardText>
                  <strong>Client:</strong> {current_order.client_name || '-'}
                </MDBCardText>
                <MDBCardText>
                  <strong>PO Number:</strong> {current_order.po_number}
                </MDBCardText>
                <MDBCardText>
                  <strong>Contract No:</strong> {current_order.contract_no}
                </MDBCardText>
              </div>
              <div className="col-md-4">
                <MDBCardText>
                  <strong>Location:</strong>{' '}
                  {current_order.location_name || '-'}
                </MDBCardText>
                <MDBCardText>
                  <strong>Generator No:</strong>{' '}
                  {current_order.generator_no || '-'}
                </MDBCardText>
                <MDBCardText>
                  <strong>Fuel Price:</strong>{' '}
                  {current_order.fuel_price != null
                    ? current_order.fuel_price
                    : '-'}
                </MDBCardText>
              </div>
              <div className="col-md-4">
                <MDBCardText>
                  ...
                  {current_order.start_date || '-'}
                </MDBCardText>
                <MDBCardText>
                  <strong>End Date:</strong>{' '}
                  {current_order.end_date || 'Active'}
                </MDBCardText>
                <MDBCardText>
                  <strong>Current Status:</strong>{' '}
                  {inverter?.inverter_status?.inverter_status_name || '-'}
                </MDBCardText>
              </div>
            </div>
          ) : (
            <p className="text-muted mb-0">
              No active or historical order found for this unit.
            </p>
          )}
        </MDBCardBody>
      </MDBCard>

      {/* 2) Complete Previous History – Orders */}
      <MDBCard className="mb-4 shadow-sm">
        <MDBCardBody>
      ...
      {/* 3) Service Records (moved BEFORE Status History) */}
      <MDBCard className="mb-4 shadow-sm">
        <MDBCardBody>
          <MDBCardTitle className="fs-5 mb-3">Service Records</MDBCardTitle>
          {service_records && service_records.length > 0 ? (
            <div className="table-responsive">
              <MDBTable hover small>
                <MDBTableHead light>
                  <tr>
            ...
            <p className="text-muted mb-0">No service records for this unit.</p>
          )}
        </MDBCardBody>
      </MDBCard>

      {/* 4) Status History (now after Service Records) */}
      <MDBCard className="mb-4 shadow-sm">
        <MDBCardBody>
